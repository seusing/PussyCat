...
it('opens only the selected batch and keeps its result snapshot across background retries', async () => {
  ...
  const detailReadsBeforeResult = calls.filter((call) => call.key === 'GET /vk/v1/jobs/a').length
  ...
  expect(calls.filter((call) => call.key === 'GET /vk/v1/jobs/a')).toHaveLength(detailReadsBeforeResult)
