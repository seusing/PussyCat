...
#### 10.2.1 执行记录 —— 2026-07-31：策略层已通过；**界面已大改，四道门需重验**

> ⚠️ **先读这段再读下表。** 本次验证跑的是 **`73ebe22` 的构建**。此后落了 16 个提交，其中
> 大量改动直接落在下表验的那些界面元素上（导航折叠、字号、中文文案、**撤销入口由下划线
> 链接改为整条状态条**、可拖拽侧栏、侧栏开关、新增登录状态模块、应用两次更名）。
>
> **仍然成立的**：策略层与执行准入的行为——八条命令的判决、确认协议、拒绝面的 Host 侧
> 拒绝、指纹漂移的失效与隔离。这些自 `73ebe22` 起一行未动，且有代码覆盖。
>
> **不再成立的**：下表中依赖界面呈现的四道门。它们验的是"用户看到了什么"，而那部分已被替换。
>
> | 门 | 为何需要重验 |
> |---|---|
> | 八条命令弹确认框 | 确认对话框与触发路径均已改动 |
> | 结果渲染 | 结果表格改用 renderCellValue，且字号与布局全变 |
> | 拒绝面置灰 | 置灰说明的呈现位置与文案已改 |
> | 撤销 | **入口形态整个换掉了**：原为运行按钮下的下划线链接，现为写明授予内容的状态条 |
>
> **另有一整块从未真机验过**：登录状态模块（后台自动执行 whoami、串行队列、
> acknowledgement 随请求提交、定时刷新）。它是一条全新的执行路径，只有单元测试。
>
> 结论：**当前 HEAD 不得声称「1a 真机闭环」**。下表保留为 `73ebe22` 那次的历史记录，
> 不是对当前版本的断言。
