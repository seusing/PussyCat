...
describe('WrssRuntimeManager', () => {
  ...
  it('patches an existing installed UI theme idempotently before each start', async () => {
    ...
    expect(themeCss).toContain('position: static !important;')
    expect(themeCss).toContain('inset: auto !important;')
    expect(themeCss).toContain('transform: none !important;')
    expect(themeCss).toContain('left: 0;')
    expect(themeCss).toContain('right: auto;')
    const uiScript = readFileSync(join(installed.sourceDir, 'static', 'pussycat-ui.js'), 'utf8')
    expect(uiScript).toContain('更多')
